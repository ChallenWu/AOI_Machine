            ...
            set
            {
                ...
                if (XTaskManager.Instance.FindTaskById(taskId) != null)
                {
                    ...
                    foreach (KeyValuePair<int, XDo> kvp in XTaskManager.Instance.FindTaskById(taskId).DoMap)
                    {
                        ...
                        dr.Cells[4].Value = MultiLanguage.GetMessage(kvp.Value.Name);//kvp.Value.Name;
                        
                        dataGridView1.Rows.Add(dr);
                        stsMap.Add(kvp.Key, DOSTSTYPE.LOW);
                        lastStsMap.Add(kvp.Key, DOSTSTYPE.LOW);
                    }
                }
            }
        }
        ...
        private void m_Timer_Tick(object sender, EventArgs e)
        {
            try
            {
                foreach (DataGridViewRow dr in dataGridView1.Rows)
                {
                    ...
                    if (stsMap[doId] == DOSTSTYPE.HIGH && lastStsMap[doId] == DOSTSTYPE.LOW)
                    {
                        dr.Cells[0].Value = Properties.Resources._lampBlue20;
                    }
                    else if (stsMap[doId] == DOSTSTYPE.LOW && lastStsMap[doId] == DOSTSTYPE.HIGH)
                    {
                        dr.Cells[0].Value = Properties.Resources._lampGray20;
                    }
                    DOSTSTYPE temp = stsMap[doId];
