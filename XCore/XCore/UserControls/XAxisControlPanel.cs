...
namespace XCore
{
    public partial class XAxisControlPanel : UserControl
    {
        ...
        public XAxisControlPanel()
        {
            ...
            LB_Home.Text = MultiLanguage.GetMessage("未初始化");//"未初始化";
            ...
            label11.Text = MultiLanguage.GetMessage("使能");
            label12.Text = MultiLanguage.GetMessage("负极限");
            label13.Text = MultiLanguage.GetMessage("原点");
            label16.Text = MultiLanguage.GetMessage("正极限");
            label15.Text = MultiLanguage.GetMessage("轴报警");
            label14.Text = MultiLanguage.GetMessage("异常停止");
        }
        ...
        public int TaskId
        {
            ...
            set
            {
                ...
                if (XTaskManager.Instance.FindTaskById(taskId) != null)
                {
                    ...
                    foreach (KeyValuePair<int, XAxis> kvp in XTaskManager.Instance.FindTaskById(taskId).AxisMap)
                    {
                        Comb_AxisNo.Items.Add(MultiLanguage.GetMessage(kvp.Value.Name));
                    }
        ...
        private void ChangeButton_Image(XAxis axisNow)
        {
            switch (axisNow.AxisDirection)
            {
                case XAxisDirection.Front_Back:
                    this.Btn_Forward.Image = XCore.Properties.Resources._up;
                    this.Btn_Back.Image = XCore.Properties.Resources._down;
                    break;
                case XAxisDirection.Left_Right:
                    this.Btn_Forward.Image = XCore.Properties.Resources._right;
                    this.Btn_Back.Image = XCore.Properties.Resources._left;
                    break;
                case XAxisDirection.Rotate:
                    this.Btn_Forward.Image = XCore.Properties.Resources._rotate_clock;
                    this.Btn_Back.Image = XCore.Properties.Resources._rotate_antiClock;
                    break;
                case XAxisDirection.Rotate_antiClock:
                    this.Btn_Forward.Image = XCore.Properties.Resources._rotate_antiClock;
                    this.Btn_Back.Image = XCore.Properties.Resources._rotate_clock;
                    break;
                case XAxisDirection.Up_Down:
                    ...
                    break;
                default:
                    this.Btn_Forward.Image = XCore.Properties.Resources._minus;
                    this.Btn_Back.Image = XCore.Properties.Resources._minus;
                    break;
            }
        }
        ...
        private void Btn_Home_Click(object sender, EventArgs e)
        {
            if (Comb_AxisNo.Items.Count > 0)
            {
                if(XDevice.Instance.FindAxisById(m_AxisId).GoHome() == 0)
                //if (XController.Instance._MoveHome(m_AxisId) == 0)
                {
                    BzMessagebox.Show(MultiLanguage.GetMessage("回零完成"));
                }
                else 
                {
                    BzMessagebox.Show(MultiLanguage.GetMessage("回零失败"));
                }
            }
        }
        ...
        private void Btn_Stop_Click(object sender, EventArgs e)
        {
            if (Comb_AxisNo.Items.Count > 0)
            {
                ...
                XDevice.Instance.FindAxisById(m_AxisId).Stop();
            }
        }
        ...
        private void m_Timer_Tick(object sender, EventArgs e)
        {
            try
            {
                if (Comb_AxisNo.Items.Count > 0)
                {
                    ...
                    bool[] axisSts = new bool[] { m_Axis.IsSVON, m_Axis.IsMEL, m_Axis.IsORG, m_Axis.IsPEL, m_Axis.IsALM, m_Axis.IsASTP, m_Axis.IsHomeOk };
                    for (int i = 0; i < 7; i++)
                    {
                        if (i < 6)
                        {
                        ...
                        else
                        {
                            if (axisSts[i] == true)
                            {
                                LB_Home.Text = MultiLanguage.GetMessage("已初始化");
                                LB_Home.ForeColor = Color.Black;
                            }
                            else
                            {
                                LB_Home.Text = MultiLanguage.GetMessage("未初始化");
                                LB_Home.ForeColor = Color.Red;
                            }
                        }
                    }
        ...
        private void bt_JOG_P_MouseDown(object sender, MouseEventArgs e)
        {
            if (Comb_AxisNo.Items.Count > 0)
            {
                ...
                if (XDevice.Instance.FindAxisById(m_AxisId).APS_SetAxisJogParam(1, 0, m_Acc, m_Acc, (int)m_Vel) != 0)
                {
                    return;
                }
                m_JogStart = 1;
        ...
        private void Comb_Distance_TextChanged(object sender, EventArgs e)
        {
            try
            {
                m_Distance = Convert.ToDouble(Comb_Distance.Text);
            }
            catch
            {
                BzMessagebox.Show(MultiLanguage.GetMessage("输入非数字字符"), MultiLanguage.GetMessage("错误"), 
        ...
        {
            XDevice.Instance.FindCardById(XDevice.Instance.FindAxisById(m_AxisId).CardId).ClearError();
        }
