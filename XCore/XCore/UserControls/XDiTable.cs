            ...
            set
            {
                ...
                if (XTaskManager.Instance.FindTaskById(taskId) != null)
                {
                    ...
                    foreach (KeyValuePair<int, XDi> kvp in XTaskManager.Instance.FindTaskById(taskId).DiMap)
                    {
                        ...
                        dr.Cells[3].Value = MultiLanguage.GetMessage(kvp.Value.Name);
                        
                        dataGridView1.Rows.Add(dr);
                        stsMap.Add(kvp.Key, DISTSTYPE.LOW);
                        lastStsMap.Add(kvp.Key, DISTSTYPE.LOW);
                    }
                }
            }
        }
        ...
        private void m_Timer_Tick(object sender, EventArgs e)
        {
            try
            {
                foreach (DataGridViewRow dr in dataGridView1.Rows)
                {
                    ...
                    if (stsMap[diId] == DISTSTYPE.HIGH && lastStsMap[diId] == DISTSTYPE.LOW)
                    {
                        dr.Cells[0].Value = Properties.Resources._lampGreen20;
                    }
                    else if (stsMap[diId] == DISTSTYPE.LOW && lastStsMap[diId] == DISTSTYPE.HIGH)
                    {
                        dr.Cells[0].Value = Properties.Resources._lampGray20;
                    }
                    DISTSTYPE temp = stsMap[diId];
