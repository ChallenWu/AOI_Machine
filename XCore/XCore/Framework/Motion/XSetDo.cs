        ...
        private int PrimOnSignal()
        {
            if (sw.ElapsedMilliseconds > 200)
            {
                for (int i = 0; i < m_Count1; i++)
                {
                    ...
                    {
                        case DOSTSTYPE.PLS:
                            XDevice.Instance.FindDoById(m_DoId1[i]).SetDo(DOSTSTYPE.LOW);
                            sw.Reset();
                            break;
                        case DOSTSTYPE.PLF:
                            XDevice.Instance.FindDoById(m_DoId1[i]).SetDo(DOSTSTYPE.HIGH);
                            sw.Reset();
                            break;
                    }
                }
        ...
        private int PrimOnSetDo()
        {
            ...
            for (int i = 0; i < m_Count; i++)
            {
                switch (m_DoStsType[i])
                {
                    case DOSTSTYPE.HIGH:
                        {
                            XDevice.Instance.FindDoById(m_DoId[i]).SetDo(DOSTSTYPE.HIGH);
                        }
                        break;
                    case DOSTSTYPE.LOW:
                        {
                            XDevice.Instance.FindDoById(m_DoId[i]).SetDo(DOSTSTYPE.LOW);
                        }
                        break;
                    case DOSTSTYPE.PLS:
                        {
