...
namespace XCore
{
    public class XMove : XCtrlEventHandler
    {
                ...
                for (int i = 0; i < m_Count; i++)
                {
                    if (m_MoveMode == MoveMode.MOVE_HOME)
                    {
                        ...
                        if (XDevice.Instance.FindAxisById(m_AxisId[i]).IsALM == true)
                        {
                            ...
                            m_Status = -2;
                            bAlarm = true;
                            PostEvent(xStation, XAlarmLevel.STOP, XAlarmId.AXIS_ALM, m_AxisId[i]);
                        }
                    }
                    else if (m_MoveMode == MoveMode.APS_PT_START)
                    {
                        ...
                        if (XDevice.Instance.FindAxisById(m_AxisId[i]).IsASTP == true)
                        {
                            ...
                            m_Status = -2;
                            bAlarm = true;
                            PostEvent(xStation, XAlarmLevel.STOP, XAlarmId.AXIS_ASTP, m_AxisId[i]);
                        }
                        ...
                        if (XDevice.Instance.FindAxisById(m_AxisId[i]).IsASTP == true)
                        {
                            if (XDevice.Instance.FindAxisById(m_AxisId[i]).IsPEL == true)
                            {
                                if (m_CheckLmt)
                                {
                                    ...
                                    m_Status = -2;
                                    bAlarm = true;
                                    PostEvent(xStation, XAlarmLevel.STOP, XAlarmId.AXIS_PEL, m_AxisId[i]);
                                }
                            }
                            else if (XDevice.Instance.FindAxisById(m_AxisId[i]).IsMEL == true)
                            {
                                if (m_CheckLmt)
                                {
                                    ...
                                    m_Status = -2;
                                    bAlarm = true;
                                    PostEvent(xStation, XAlarmLevel.STOP, XAlarmId.AXIS_MEL, m_AxisId[i]);
                                }
                            }
                            else
                            {
        ...
        private int PrimOnMoveJog()
        {
            ...
            for (int i = 0; i < m_Count; i++)
            {
                XDevice.Instance.FindAxisById(m_AxisId[i]).MoveJog(m_isStart[i],30);
            }
            m_Move = true;
            ...
            if (ret != 0)
            {
                if (m_ServoSts == true)
                {
                    for (int i = 0; i < count; i++)
                    {
                        if (XDevice.Instance.FindAxisById(m_AxisId[i]).IsSVON == false)
                        {
                            PostEvent(xStation, XAlarmLevel.STOP, XAlarmId.AXIS_SERVON_FAIL, m_AxisId[i]);
                        }
                    }
                }
            }
