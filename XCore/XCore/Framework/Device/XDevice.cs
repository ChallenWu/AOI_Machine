...
namespace XCore
{
    public sealed class XDevice : XObject
    {
        ...
        public void DeleteCard(int setCardId)
        {
            if (cardMap.ContainsKey(setCardId) == true)
            {
                cardMap.Remove(setCardId);
            }
        }
        ...
        public List<XCard> FindCardByCardType(CardType cardtype)
        {
            List<XCard> CardList = new List<XCard>();
            foreach (KeyValuePair<int,XCard> kvp in cardMap)
            {
                if (kvp.Value.CardType == cardtype)
                    CardList.Add(kvp.Value);
            }
            return CardList;
        }
        ...
        public void BindAxis(int setCardId, int setAxisId, int actAxisId, double lead, string name,XAxisDirection axisDirection,bool chklimit = true)
        {
            if (cardMap.ContainsKey(setCardId) == true)
            {
                if (axisMap.ContainsKey(setAxisId) == false)
                {
                    XAxis axis = new XAxis(actAxisId, lead, cardMap[setCardId], name,chklimit);
        ...
        public XAxis FindAxisByActId(int actCardId, int actAxisId, CardType cardType)
        {
            //for (int i = 0; i < axisMap.Count; i++)
            //{
            //    if (axisMap[i].ActId == actAxisId && XDevice.Instance.FindCardById(axisMap[i].CardId).ActId == actCardId)
            //        return axisMap[i];
            //}
            foreach (KeyValuePair<int, XAxis> kvp in axisMap)
            {
                if (cardType != XDevice.Instance.FindCardById(kvp.Value.CardId).CardType)
                    continue;
                if (kvp.Value.ActId == actAxisId && XDevice.Instance.FindCardById(kvp.Value.CardId).ActId == actCardId)
                    return kvp.Value;
            }
            return null;
        }
