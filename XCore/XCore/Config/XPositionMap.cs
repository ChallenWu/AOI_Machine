    ...
    public class XPositionMap : XObject
    {
        ...
        public int LoadPositions()
        {
            try
            {
                ...
                int result = XXml.ReadNodeAndInnerText(path, root + "//" + node, out nodes1, out content1);
        ...
        public int LoadPositionNames()
        {
            try
            {
                nameMap.Clear();
                string[] nodes1, content1;
                int result = XXml.ReadNodeAndInnerText(name_path, root + "//" + node, out nodes1, out content1);
                if (result >=0 && nodes1.Length > 0)
                {
