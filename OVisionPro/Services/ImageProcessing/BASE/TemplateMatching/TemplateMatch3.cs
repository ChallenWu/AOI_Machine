...
namespace OVisionPro.Services.ImageProcessing.BASE.TemplateMatching
{
    public class TemplateMatch3
    {
        ...
        int Count;
        Dictionary<string, PatternData> patternData = new Dictionary<string, PatternData>();
        public (bool, Dictionary<string, object>) TemplMatchO(Mat sourceImage, string blockID)
        {
            ...
            try
            {
                ...
                {
                    if (!patternData.ContainsKey(blockID))
                    {
                        Mat patternImg = XImgPatternManager.Instance.ImgMatPatternData[blockID];
                        PatternData patternData1 = new PatternData(patternImg, new Point3f((float)(patternImg.Height / 2), (float)(patternImg.Width / 2), 0.0f));
                        patternData.Add(blockID, patternData1);
                    }
                    if (patternData[blockID].PatternImage.IsDisposed)
                    {
                        Mat patternImg = XImgPatternManager.Instance.ImgMatPatternData[blockID];
                        patternData[blockID] = new PatternData(patternImg, new Point3f((float)(patternImg.Height / 2), (float)(patternImg.Width / 2), 0.0f));
                    }
                    if (OBasicAlgorithm.IsMat(sourceImage) && OBasicAlgorithm.IsMat(patternData[blockID].PatternImage) && OBasicAlgorithm.checkInputsMatch(patternData[blockID].PatternImage, sourceImage))
                    {
                        Point3f patternCP = (OBasicAlgorithm.IsMat(patternData[blockID].PatternImage)) ? new Point3f((float)(patternData[blockID].PatternImage.Height / 2), (float)(patternData[blockID].PatternImage.Width / 2), 0.0f) : new Point3f(0, 0, 0.0f);
                        ...
                        {
                            double num6 = num3;
                            TempleMatch_(sourceImage, patternData[blockID], searchRegionAdaptive, num3, num2, num, num4, out matchingResult, margin);
                            searchRegionAdaptive = new Rect((int)Math.Round((double)matchingResult.CP.X - 1.5 * (double)patternData[blockID].PatternImage.Width / 2.0), (int)Math.Round((double)matchingResult.CP.Y - 1.5 * (double)patternData[blockID].PatternImage.Height / 2.0), (int)Math.Round(1.5 * (double)patternData[blockID].PatternImage.Width), (int)Math.Round(1.5 * (double)patternData[blockID].PatternImage.Height));
                            Count++;
                        ...
                        {
                            searchRegionAdaptive = new Rect((int)Math.Round(matchingResult.CP.X - (float)(patternData[blockID].PatternImage.Width / 2)), (int)Math.Round(matchingResult.CP.Y - (float)(patternData[blockID].PatternImage.Height / 2)), patternData[blockID].PatternImage.Width, patternData[blockID].PatternImage.Height);
                            margin = 1.1;
                            TempleMatch_(sourceImage, patternData[blockID], searchRegionAdaptive, num3, 0.0, num, num4, out matchingResult, margin, maskedScore: true);
                        }
                        num3 = (double)matchingResult.CP.Z * 180.0 / Math.PI;
                        num = num2 / 2.0;
                        num4 = 1.0 / ScaleLast;
                        searchRegionAdaptive = new Rect((int)Math.Round((double)matchingResult.CP.X - 1.1 * (double)patternData[blockID].PatternImage.Width / 2.0), (int)Math.Round((double)matchingResult.CP.Y - 1.1 * (double)patternData[blockID].PatternImage.Height / 2.0), (int)Math.Round(1.1 * (double)patternData[blockID].PatternImage.Width), (int)Math.Round(1.1 * (double)patternData[blockID].PatternImage.Height));
                        margin = 1.1;
                        bool result = TempleMatch_(sourceImage, patternData[blockID], searchRegionAdaptive, num3, num2, num, num4, out matchingResult, margin, maskedScore: false, fineFind: true);
                        float num7 = patternData[blockID].PatternCP.X;
                        float num8 = patternData[blockID].PatternCP.Y;
                        ...
                        Cv2.Circle(sourceImage, new OpenCvSharp.Point(cp.X, cp.Y), 3, new Scalar(0, 110, 255), 5);
                        //frame = RotationBound(frame, num3);
                        externVal["angle"] = num3;
                        externVal["center"] = new Point2f(cp.X, cp.Y);
                        externVal["msg"] = "OK";

                        Cv2.PutText(sourceImage, externVal["msg"].ToString(), fontPos, fontT, fontScale, colorScarOK, fontThickness);
                        cvX.ImShow(blockID, "RES_DISPLAY", sourceImage);
                        Mat test = RotationBound(sourceImage, (double)externVal["angle"]);
                        test?.Dispose();
                        return (true, externVal);
                    }
            ...
            {
                resCheck = false;
                externVal["msg"] = "LOI TOOL : " + ex.Message.ToString();
            } 
        ...
        private bool TempleMatch_(Mat sourceImage, PatternData patternData, Rect searchRegionAdaptive, double degStartAdaptive, double degRangeAdaptive, double degStepAdaptive, double searchScaleAdaptive, out MatchingResult matchingResult, double margin = 1.5, bool maskedScore = false, bool fineFind = false)
        {
            ...
            for (double num8 = num6; num8 <= num7; num8 += degStepAdaptive)
            {
                ...
                Mat abc = ApplyMask(mat5, transferMat, out var mask);
                Cv2.MinMaxLoc(abc, out minVal, out maxVal, out minLoc, out maxLoc, mask);
                mat5?.Dispose();
                mask?.Dispose();
                abc.Dispose();
                double num9 = 0.9;
        ...
        public (Mat, OpenCvSharp.Point) RotationAngleBound(Mat frame, Point2f orgPoint, double angle, double scale = 1.0)
        {
            ...
            rotationMatrix.Set(0, 2, rotationMatrix.At<double>(0, 2) + ((newWidth / 2.0) - center.X));
            rotationMatrix.Set(1, 2, rotationMatrix.At<double>(1, 2) + ((newHeight / 2.0) - center.Y));
            int xN = (int)(orgPoint.X * cos - orgPoint.Y * sin);
            int yN = (int)(orgPoint.X * sin + orgPoint.Y * cos);
            OpenCvSharp.Point newP = new OpenCvSharp.Point(xN, yN);
